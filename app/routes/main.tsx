...
export default function Main() {
  ...
  for (let i: number = 0; i < products.length; i += 4) {
    componentArray[i] = (
      <Item
        key={i}
        image={products[i].productName + ".jpg"}
        priceS={products[i].productPrice}
        priceM={products[i + 1].productPrice}
        priceL={products[i + 2].productPrice}
        priceXl={products[i + 3].productPrice}
        art_name={products[i].productName}
      />
    );
  }
